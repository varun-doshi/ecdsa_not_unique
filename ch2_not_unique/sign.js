...
function sign(privateKey, hash, nonce) {
  ...
  const sigObj = ecdsaSign(
    ...
    new Uint8Array(Buffer.from(util.removeLeading0x(privateKey), "hex")),
    {
      ...
      noncefn: (message, privateKey, algo, data, attempt) => {
        return new Uint8Array(Buffer.from(util.removeLeading0x(nonce), "hex"));
      },
    }
  );
...
const privateKey =
  "0xe8f2ae1b4e2932271ac9fab5dac2b0aa83498376e969c915b47863a85bb91a9a";
// corresponding address: "0xF6A2dc49a58C3260e2AF963A44e36F8C4332a640";
const message =
  "0x62ba5d9a955135bd710cbe6cf9f35a15364ab928f278104c682912d096811a7d";

// generating first signature.
let nonce = _hash.keccak256("John", "001");
const sig1 = sign(privateKey, message, nonce);
console.log(`sig1: ${sig1}`);

// generating a new signature with a different nonce.
nonce = _hash.keccak256("Jason", "002");
